...
cps <- load_org(1994:2019, year, month, orgwgt, age, emp, lfstat, selfemp, selfinc, hoursu1, hoursu1i, hoursvary, paidhre, wage, wageotc, weekpay, mind16, mocc10) %>%
  ...
  mutate(realwage = wage*(cpi_base/cpiurs),
         #Constructed weekly pay variable
         c.realweekpay.pre = ifelse(hours>0, yes=hours * realwage, NA),
         c.realweekpay.post = hours * realwage) %>% 
  ...
  mutate(pwages = ifelse(realwage>0 & !is.na(realwage) , yes=1, no=0), 
         pweekpay.c = ifelse(c.realweekpay.pre>0 & !is.na(c.realweekpay.pre) , yes=1, no=0),
...
set.seed(1017)
##assign each observation a random # between 0 and 1
...
filteredcps <- cps %>% 
  ...
  mutate(index = runif(nrow(.), min = 0, max=1)) %>%  
  #sort by year, real weekly pay
  arrange(year, !is.na(c.realweekpay.pre), c.realweekpay.pre, index) %>%
  group_by(year) %>% 
  #column with cumulative sum of org weight
...
med_cutoffs <- filteredcps %>% 
  ...
  summarize(avg_weekpay_cutoff = max(realweekpay.c, na.rm=TRUE)) %>% 
  pivot_wider(id_cols = year, names_from = med_bins, values_from = avg_weekpay_cutoff)
  
quintile_metrics <- filteredcps %>%
  group_by(year, quint_bins) %>%
  summarize(avg_weekpay = weighted.mean(realweekpay.c, na.rm=TRUE),
...
remaining_samp <- cps %>% 
  ...
  mutate(realwage0 = replace(realwage, emp==0, -999),
         realweekpay0 = replace(realweekpay.c, emp==0, -999),
...
create indicator for two groups unemp and emp
...
saveWorkbook(rand_order, here(paste0("output/metrics_", format(Sys.time(), "%d-%b-%Y %H.%M"), ".xlsx")))
```

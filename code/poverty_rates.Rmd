...
cps <-load_basic(2020, year, month, ftptstat, selfemp, selfinc, personid, hhid, hrhhid, hrhhid2, hhtype, famtype, famid, famrel, ownchild, agechild, wage, weekpay, ind17, mind03, occ18, hourslwt, hourslw1, emp, lfstat, orgwgt, finalwgt, basicwgt, age, female, wbhao, faminc, minsamp, cow1) %>% 
  ...
  mutate(inflation_dec2020 = (cpi_base/cpi_u_rs)-1,
         realweekpay = weekpay*(cpi_base/cpi_u_rs),
         realweekpay0 = replace(realweekpay, hourslw1==0, 0), #if not at work last week, wages==0
         all_wgt = ifelse(age<16, yes=finalwgt, no=orgwgt), #create a new weight for those under 16
         headratio = ifelse(famrel==1, yes=wgt/finalwgt, no=NA), #create ratio for primary respondent
         realyearpay0 = realweekpay0 *52,
...
#benchmarked
adults <- cps %>% 
  summarize(adults_16plus = sum(wgt, na.rm=TRUE),
            adults_pwages = sum(wgt[weekpay>0], na.rm=TRUE),
            n = n())
...
adults_month <- cps %>% 
  ...
  summarize(adults_16plus = sum(orgwgt, na.rm=TRUE),
            adults_elig = sum(orgwgt[cow1 %in% c(1,2)]),

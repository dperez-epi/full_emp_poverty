...
cpsorg <-load_org(1994:2019, year, month, orgwgt, age, emp, lfstat, selfemp, selfinc, hourslw1, hoursu1i, hoursuorg, hoursut, hoursvary, wage, weekpay, mind16, mocc10, cow1, dlfstat, emphrs) %>%
  ...
  mutate(hours = replace(hours, emp==0, 0), #Assign unemp/NILF workers 0 hours
         ...
         pweekpay2 = ifelse(realweekpay2>0 & !is.na(realweekpay2) , yes=1, no=0),
  ...
  set_value_labels(pwages = c('Positive wages' = 1, 'Zero wages' = 0),
                   pweekpay2 = c('Positive weekpay' = 1, 'Zero weekpay' = 0),
...
emp_cuts <- cpsorg %>% 
#Calc average wages, hours, and weekly pay for all workers with positive weekly pay indicator
  filter(emp==1, pweekpay==1) %>% 
  group_by(year) %>% 
  summarize(avg_realwage = weighted.mean(realwage, w=wgt, na.rm=TRUE),
            avg_hours = weighted.mean(hours, w=wgt, na.rm=TRUE),
            avg_realweekpay = weighted.mean(realweekpay2, w=wgt, na.rm=TRUE)) %>%
  mutate(wageXhours = avg_realwage * avg_hours,
         diff = avg_realweekpay - wageXhours)
  write.xlsx(here('data/emp_cuts.xlsx'))

#Crosstab of employed worker hours, wages, and weekly pay indicators
test<- cpsorg %>% 
  filter(emp==1) %>% 
  mutate(across(phours1|pwages|pweekpay|pweekpay2, ~to_factor(.x))) %>% 
